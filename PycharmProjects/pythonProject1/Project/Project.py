...
def number():
    get_random_number = random.randint(1, 151)
    return get_random_number

def get_pokemon(id):
    url = 'https://pokeapi.co/api/v2/pokemon/{}/'.format(id)
    response = requests.get(url).json()
    pokemon = {'ID':response['id'], 'name':response['name'], 'height':response['height'], 'weight':response['weight']}
    return pokemon

player_id = number()

players_pokemon = get_pokemon(player_id)
print('the trumps on your pokemon are {}'.format(players_pokemon))

opponent_id = number()

opponents_pokemon = get_pokemon(opponent_id)

print('lets play')

stat_choice = input('what stat would you like to use?')

if players_pokemon[stat_choice] > opponents_pokemon[stat_choice]:
    print('i win')
elif players_pokemon[stat_choice] < opponents_pokemon[stat_choice]:
    print('you win')
else:
    print('draw')